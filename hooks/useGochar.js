...
export const useGochar = (input) => {
  const [planets, setPlanets] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);

  const fetchGochar = useCallback(async () => {
    if (!input) return;
    try {
      setLoading(true);
      setError(null);
      const response = await astroGet('horoscope/planet-details', input);
      setPlanets(response?.response ?? null);
    } catch (e) {
      setError(e.message);
      setPlanets(null);
    } finally {
