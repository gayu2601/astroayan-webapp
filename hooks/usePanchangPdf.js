...
import { generatePanchangHTML } from '../data/panchangLogic';
...
// Flopes-corrected Map: Identifies the exact BIRTH STARS experiencing Chandrashtama when Moon sits in Rasi Index
...
export const usePanchangPdf = () => {
  const [htmlObj, setHtmlObj] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error,   setError]   = useState(null);
  const { user } = useAuth();

  const fetchPanchangData = async (input) => {
	  console.log('in fetchPanchangData', input)
    if (!input) return;
    setLoading(true);
    setError(null);
    const { day, month, year, hour, min, tzone, lat, lon, lang } = input;
    const currentLang = lang || 'ta';
    ...
    try {
      ...
      const planetParams = { ...rest, dob: paddedDate, tob: paddedTime };
      
      const current = new Date(input.year, input.month - 1, input.day);
      ...
      const apiSunset = panToday?.response?.sunset || "06:30 PM";
		const computedGowriBlocks = calculateDynamicGowriNeram(apiSunrise, apiSunset, dayNameInEnglish, currentLang);
		const derivedChandrashtamaStars = calculateOfflineChandrashtamaStars(input, currentLang);
		let dataObj = {panchang:        panToday,planets:         planToday,gowriNallaNeram: computedGowriBlocks, chandrashtama:   derivedChandrashtamaStars,standardNallaNeram};
		console.log('dataObj', dataObj)
		const html = generatePanchangHTML(dataObj, currentLang, user);
		console.log(html);
		if (Platform.OS === 'web') {
		  ...
		  // Pull out just the <style> block and the inner body markup from the
		  // single self-contained document generatePanchangHTML returns, so we
		  // can merge them into ONE document instead of nesting two <html> trees.
		  const styleMatch = html.match(/<style>([\s\S]*?)<\/style>/);
		  const bodyMatch = html.match(/<body[^>]*>([\s\S]*?)<\/body>/);
		  const panchangStyles = styleMatch ? styleMatch[1] : '';
		  const panchangBody = bodyMatch ? bodyMatch[1] : html;
			...
			<body>
			  ...
			  <script>
				async function shareReport() {
				  ...
				  // Capture at the content's own natural (mobile-responsive) width
				  // instead of forcing a fixed A4 size — keeps the shared image
				  // looking like the on-screen card, not a stretched print page.
				  const contentWidth = original.querySelector('.page-container')
					? original.querySelector('.page-container').offsetWidth
					...
					if (navigator.canShare && navigator.canShare({ files: [file] })) {
					  try {
						await navigator.share({
						  files: [file],
						  title: 'பஞ்சாங்கம்',
						  text: '🕉️ **இன்றைய பஞ்சாங்கம்** – திதி, நட்சத்திரம், நல்ல நேரம், ராகு காலம் உள்ளிட்ட முழுமையான பஞ்சாங்க விவரங்கள். 📲 தினமும் இலவசமாக **AstroAyan** செயலியில் அறிந்து பயன்பெறுங்கள்: https://astroayan.com'
						});
					  } catch (e) {}
					} else {
  ...
  return {fetchPanchangData, loading, error};
};