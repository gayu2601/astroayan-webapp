          ...
          <Routes>
            ...
            <Route path="/" element={
              <div className="space-y-8 animate-fade-in">
                ...
                <div className="gradient-group-tools p-6 rounded-3xl space-y-5 animate-fade-in border">
                  <div className="border-b border-gray-700/10 pb-4">
                    <h2 className="text-xl font-serif font-black tracking-tight text-amber-500 flex items-center gap-2"><Sparkles className="h-5 w-5" />{language === 'ta' ? "பிரசன்னம்" : "Prasannam"}</h2>
                    <p className={`text-xs mt-1 ${isLight ? "text-[#5C4F43]" : "text-gray-400"}`}>{language === 'ta' ? "ஜாமக்கோள், சோழி மற்றும் வெற்றிலை பிரசன்ன முறைகள் மூலம் உங்கள் கேள்விகளுக்கான விடைகளை அறியுங்கள்." : "Seek answers through traditional Jamakkol, Choli, and Vetrilai Prasannam divination methods."}</p>
                  </div>
            ...
            {/* ── PRASANNAM (new tab) ─────────────────────────────────────────────── */}
            <Route path="/prasannam/*" element={
              <div className="space-y-6 animate-fade-in">
                <BackButton />
                <div className="flex flex-wrap gap-2 border-b border-gray-700/25 pb-3">
                  {[
                    { id: 'jamakkol',          labelEn: 'Jamakkol Prasannam',  labelTa: 'ஜாமக்கோள் பிரசன்னம்' },
                    { id: 'choli',             labelEn: 'Choli Prasannam',     labelTa: 'சோழி பிரசன்னம்' },
                    { id: 'vetrilai',          labelEn: 'Vetrilai Prasannam',  labelTa: 'வெற்றிலைப் பிரசன்னம்' },
                  ].map((sub) => (
                    <button key={sub.id} onClick={() => navigate(`/prasannam/${sub.id}`)} className={subPillClass(prasannamSubTab === sub.id)}>
                      {language === 'ta' ? sub.labelTa : sub.labelEn}
                    </button>
                  ))}
                </div>
                ...
                {vedicToolSub === null ? (
                  <div className="space-y-6">
                    <div className="border-b border-gray-700/25 pb-3">
                      <h2 className="text-xl font-serif font-bold text-amber-500">{language === 'ta' ? "கருவிகள்" : "Other Tools"}</h2>
