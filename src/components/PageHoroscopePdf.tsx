...
interface PageHoroscopePdfProps {
  isLight?: boolean;
}

export default function PageHoroscopePdf({ isLight = true }: PageHoroscopePdfProps) {
            ...
            <div className="space-y-6">
              ...
              {reportData && (
                <div className={`rounded-xl p-4 border space-y-2 text-xs transition-all ${isLight ? "bg-amber-50/20 border-amber-500/10 text-gray-700" : "bg-white/5 border-white/5 text-gray-300"}`}>
                  ...
                  <div className="flex justify-between">
                    <span className={`font-medium ${isLight ? "text-gray-500" : "text-gray-400"}`}>{language === 'ta' ? 'லக்னம்:' : 'Lagna:'}</span>
                    <span className={`font-semibold ${isLight ? "text-gray-900" : "text-white"}`}>{reportData.astro?.ascendant_sign || '—'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={`font-medium ${isLight ? "text-gray-500" : "text-gray-400"}`}>{language === 'ta' ? 'ராசி:' : 'Rashi:'}</span>
                    <span className={`font-semibold ${isLight ? "text-gray-900" : "text-white"}`}>{reportData.astro?.rasi || '—'}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className={`font-medium ${isLight ? "text-gray-500" : "text-gray-400"}`}>{language === 'ta' ? 'நட்சத்திரம்:' : 'Star:'}</span>
                    <span className={`font-semibold ${isLight ? "text-gray-900" : "text-white"}`}>{reportData.astro?.nakshatra || '—'}</span>
                  </div>
                </div>
              )}
