...
export const StarGemstoneMapping: React.FC<StarGemstoneProps> = ({
  language
}) => {
  ...
  const filteredData = FINAL_DATA.filter((item) => {
    ...
    return (
      item.stars.toLowerCase().includes(q) ||
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-2xl sm:text-3xl font-bold font-serif text-amber-200">
                  {isTa
                    ? 'நட்சத்திரம் - கிரகம் - அதிர்ஷ்ட கல்'
                    : 'Natchathiram - Graham - Gemstone'}
                </h2>

                <p className="text-xs sm:text-sm text-[var(--theme-text-muted)] font-mono uppercase tracking-wider">
                  {isTa
                    ? '27 நட்சத்திரங்கள், அதிபதி கிரகங்கள் மற்றும் உகந்த அதிர்ஷ்ட ரத்தினக் கற்கள் அட்டவணை'
                    : 'Mapping of 27 Nakshatras, Ruling Planets & Lucky Gemstones'}
                </p>
              </div>
        ...
        <div className="glass-panel p-4 rounded-2xl border border-white/10 flex flex-col sm:flex-row gap-3 items-stretch sm:items-center justify-between">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder={
                isTa
                  ? 'நட்சத்திரம், கிரகம் அல்லது கல் தேடுக...'
                  : 'Search star, planet or gemstone...'
              }
                ...
                <thead>
                  <tr className="bg-black/40 border-b border-white/15 text-xs font-serif uppercase tracking-wider text-amber-300">
                    ...
                    <th className="py-4 px-5">
                      {isTa ? 'நட்சத்திரங்கள் (Stars)' : 'Stars'}
                    </th>

                    <th className="py-4 px-5">
                      {isTa ? 'அதிபதி கிரகம் (Planet)' : 'Ruling Planet'}
                    </th>

                    <th className="py-4 px-5">
                      {isTa ? 'அதிர்ஷ்ட கல் (Gemstone)' : 'Gemstone'}
                    </th>
                  ...
                  {filteredData.map((item, idx) => (
                    ...
                    >
                      ...
                      <td className="py-3.5 px-5">
                        ...
                        <span
                          className="inline-flex items-center px-2.5 py-1 rounded-lg bg-purple-500/10 border border-purple-500/30 font-bold text-xs"
                      ...
                      <td className="py-3.5 px-5">
                        ...
                        <span
                          className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-xl border ${item.colorBg} ${item.colorBorder} font-serif font-bold text-xs sm:text-sm shadow-sm`}
                          style={{ color: item.colorHex }}
                        >
            ...
            {filteredData.map((item, idx) => (
              ...
              >
                ...
                <div className="flex items-center justify-between border-b border-white/10 pb-3">
                  ...
                  {/* Bright Planet Badge */}
                  <span
                    className="inline-flex items-center px-2.5 py-1 rounded-lg bg-black/50 border border-white/15 font-serif font-bold text-xs"
                    style={{ color: item.colorHex }}
                  >
                    {item.planet}
                  </span>
                ...
                <div className="grid grid-cols-2 gap-2 pt-2 border-t border-white/5">
                  ...
                  {/* Gemstone */}
                  <div
                    className={`p-2.5 rounded-xl bg-black/40 border ${item.colorBorder} space-y-0.5`}
                  >
