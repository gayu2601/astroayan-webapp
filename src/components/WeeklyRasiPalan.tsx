...
import { useWeeklyPalan } from "../../hooks/useWeeklyPalan";
...
interface WeeklyRasiPalanProps {
  isLight?: boolean;
}
...
function getWeekDateRange(weekOffset = 0): string {
  const now = new Date();
...
export default function WeeklyRasiPalan({ isLight = false }: WeeklyRasiPalanProps) {
  ...
  const { weeklyPalan: data, loading, error, refetch } = useWeeklyPalan(hookInput);
  ...
  return (
  <ScreenGuard featureId="weekly_rasi_palan">
    <div id="weekly-rasi-palan-section" className={`p-6 rounded-3xl space-y-6 border transition-all duration-300 ${
      isLight 
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-700/25 pb-4">
        <div>
          <h2 className="text-xl md:text-2xl font-serif font-black text-amber-500 flex items-center gap-2">
            <CalendarDays className="h-6 w-6 text-amber-500 animate-pulse" />
            {t("weeklyRasi.title")}
          </h2>
          <p className="text-xs text-gray-400 mt-1">
            {isTamil ? "நடப்பு வாரம்" : "Current Week"}: {getWeekDateRange()}
          </p>
        </div>
      ...
      {!selectedZodiac && (
        <div className="flex flex-col items-center justify-center py-12 text-center text-gray-500">
          <CalendarDays className="h-10 w-10 text-gray-600 mb-2.5 animate-bounce" />
          <p className="text-sm font-medium">{t("weeklyRasi.selectPrompt")}</p>
        </div>
      )}
      ...
      {selectedZodiac && loading && (
        <div className="flex flex-col items-center justify-center py-16 text-center text-amber-500 space-y-3">
          ...
          <p className="text-sm font-bold tracking-wide animate-pulse">{t("weeklyRasi.loading")}</p>
        </div>
      )}
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex justify-between items-start mb-4">
              ...
              <div className="text-right">
                ...
                <span className="text-[9px] px-2 py-0.5 rounded-full bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 font-bold uppercase tracking-wider block mt-1">
                  {palanContent.totalScore >= 75 ? (isTamil ? "வாரம் உத்தமம்" : "Excellent Week") : palanContent.totalScore >= 55 ? (isTamil ? "வாரம் நன்று" : "Good Week") : (isTamil ? "கவனம் தேவை" : "Caution Period")}
                </span>
              </div>
            </div>
          ...
          <div className={`p-5 rounded-2xl border space-y-4 ${isLight ? "bg-white border-[#E8DCC4]" : "bg-black/30 border-white/5"}`}>
            <h3 className="text-xs font-black text-amber-500 uppercase tracking-widest flex items-center gap-1.5 border-b border-gray-700/25 pb-2">
              ...
              {isTamil ? "வாராந்திர பார்வை" : "Weekly Forecast Overview"}
            </h3>
