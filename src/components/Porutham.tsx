...
const COLOR_MAPS: Record<string, { bg: string; text: string; border: string }> = {
  good: { bg: 'bg-emerald-950/40', text: 'text-emerald-400', border: 'border-emerald-500/20' },
  med:  { bg: 'bg-amber-950/40',   text: 'text-amber-400',   border: 'border-amber-500/20' },
  bad:  { bg: 'bg-red-950/40',     text: 'text-red-400',     border: 'border-red-500/20' },
};
...
const RASI_BADGE_STYLE: Record<string, { bg: string; text: string }> = {
  'சூரியன்':  { bg: 'bg-yellow-500/20', text: 'text-yellow-300' },
  'சந்திரன்': { bg: 'bg-gray-500/20', text: 'text-gray-300' },
  'செவ்வாய்': { bg: 'bg-red-500/20', text: 'text-red-300' },
  'புதன்':    { bg: 'bg-green-500/20', text: 'text-green-300' },
  'குரு':     { bg: 'bg-amber-500/20', text: 'text-amber-300' },
  'சுக்கிரன்':{ bg: 'bg-pink-500/20', text: 'text-pink-300' },
  'சனி':      { bg: 'bg-violet-500/20', text: 'text-violet-300' },
  'ராகு':     { bg: 'bg-emerald-500/20', text: 'text-emerald-300' },
  'கேது':     { bg: 'bg-slate-500/20', text: 'text-slate-300' },
};
...
function SingleKattam({ planets = {}, accentClass, label }: SingleKattamProps) {
  ...
  const renderCell = (rasiNo: number | null, index: number) => {
    ...
    return (
      <div
        key={index}
        className={`border border-violet-500/10 p-1 flex flex-col justify-between min-h-[60px] aspect-square text-center ${
          isCenter ? 'bg-slate-950/40 col-span-2 row-span-2 flex items-center justify-center' : 'bg-slate-950/80'
        }`}
      >
        {!isCenter && rasiNo && (
          <>
            <span className="text-[10px] font-sans text-gray-500 font-semibold">{rasiNo}</span>
            <span className="text-[10px] text-gray-400 font-bold leading-none">{RASI_TAMIL_NAMES[rasiNo - 1]}</span>
            <div className="flex flex-wrap gap-0.5 justify-center items-center mt-auto mb-auto">
              {planetNames.map((name, i) => {
                const colors = RASI_BADGE_STYLE[name] || { bg: 'bg-violet-500/10', text: 'text-violet-400' };
                return (
  ...
  return (
    <div className="bg-slate-900/20 border border-gray-800 rounded-xl p-4 flex flex-col items-center w-full max-w-[340px] mx-auto space-y-2">
      <h3 className={`text-sm font-serif font-bold ${accentClass}`}>{label}</h3>
      <div className="grid grid-cols-4 grid-rows-4 border border-violet-500/20 rounded-lg overflow-hidden bg-slate-950 aspect-square w-full">
        {/* Row 0 */}
        {renderCell(12, 0)}
        {renderCell(1, 1)}
        {renderCell(2, 2)}
...
export default function Porutham() {
  ...
  const verdictColor =
    tab === 'time'
      ? score >= 24 ? 'text-emerald-400' : score >= 18 ? 'text-amber-400' : 'text-red-400'
  ...
  <ScreenGuard featureId="porutham_mrg">
    <div className="space-y-6 max-w-4xl mx-auto pb-12">
      {/* ── Form Card ── */}
      <div className="bg-slate-900/40 border border-gray-800 rounded-xl p-5 shadow-xl backdrop-blur-md space-y-5">
        ...
        {/* 2-Tab Switcher */}
        <div className="flex bg-slate-950/60 p-1 rounded-lg border border-gray-800 max-w-md">
          <button
            onClick={() => { setTab('star'); setApiInput(null); }}
            className={`flex-1 py-2 text-xs font-semibold rounded-md transition-all ${
              tab === 'star' ? 'bg-violet-600 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-2 text-xs font-semibold rounded-md transition-all ${
              tab === 'time' ? 'bg-violet-600 text-white' : 'text-gray-400 hover:text-white'
            }`}
          >
        ...
        {tab === 'star' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full bg-slate-950/60 border border-gray-800 focus:border-violet-500 focus:ring-1 focus:ring-violet-500/30 rounded-lg px-3 py-2 text-white text-sm outline-none transition-all"
              >
          ...
          <div className="space-y-4">
            ...
            <div className="border-l-2 border-pink-500 pl-3 space-y-3">
              <h3 className="text-xs font-extrabold text-pink-400 uppercase tracking-wider font-sans">
                👩 {t('porutham.girlDetails') || 'Girl\'s Birth Details'}
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="space-y-1">
                  <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Birth Date</span>
                  <input
                    type="date"
                    value={girlDob}
                ...
                <div className="space-y-1">
                  <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Birth Time</span>
                  <input
                    type="time"
                    value={girlTob}
                ...
                <div className="space-y-1">
                  <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Birth Place</span>
                  <input
                    type="text"
                    value={girlPlace}
                    onChange={(e) => setGirlPlace(e.target.value)}
                    placeholder={t('porutham.girlPlaceLabel') || 'e.g. Chennai, Tamil Nadu'}
                    className="w-full bg-slate-950/60 border border-gray-800 rounded-lg px-3 py-2 text-white text-xs outline-none focus:border-violet-500 placeholder:text-gray-600"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="border-l-2 border-blue-500 pl-3 space-y-3">
              <h3 className="text-xs font-extrabold text-blue-400 uppercase tracking-wider font-sans">
      ...
      {/* ── Error Banner ── */}
      {error && !loading && (
        <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex items-center gap-2.5 text-xs text-red-400">
      ...
      {result && !loading && !error && (
        <div className="space-y-6 animate-fade-in">
          {/* Main Score Card */}
          <div className="bg-slate-900/40 border border-gray-800 rounded-xl p-5 shadow-xl backdrop-blur-md">
            <h3 className="text-xs font-semibold tracking-wider uppercase text-amber-400 border-b border-gray-800/60 pb-2 mb-4 font-sans">
              {t('porutham.result') || 'Matching Score & Verdict'}
            </h3>

            <div className="flex flex-col sm:flex-row items-center gap-5">
              {/* Giant Radial/Circle Score Emblem */}
              <div className="w-24 h-24 rounded-full bg-slate-950 border-4 border-violet-500/30 flex flex-col items-center justify-center relative shadow-lg">
                <span className="text-2xl font-serif font-black text-amber-400 leading-none">{score}</span>
                <span className="text-[10px] text-gray-500 font-sans mt-0.5 font-bold uppercase tracking-wider">
                  / {tab === 'time' ? 36 : 10}
                </span>
              </div>
          ...
          <div className="bg-slate-900/40 border border-gray-800 rounded-xl p-5 shadow-xl backdrop-blur-md space-y-4">
            <h3 className="text-xs font-semibold tracking-wider uppercase text-amber-400 border-b border-gray-800/60 pb-2 font-sans">
                  ...
                  <div key={r.key} className="py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <span className="text-sm font-bold text-white font-serif">{r.name}</span>
                        <span className={`text-[9px] uppercase tracking-wider font-extrabold px-1.5 py-0.5 rounded ${importanceBadgeStyle(importance)}`}>
                          {importance}
                        </span>
                      </div>
                      <p className="text-xs text-gray-400 leading-normal">{r.description}</p>
                    </div>

                    <div className="flex items-center gap-3">
                      <span className="text-xs font-semibold font-mono text-gray-400">{val} / {r.full_score}</span>
          ...
          {(result.boy_astro_details || result.girl_astro_details) && (
            <div className="bg-slate-900/40 border border-gray-800 rounded-xl p-5 shadow-xl backdrop-blur-md space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                {/* Girl Details */}
                <div className="bg-slate-950/45 border border-pink-500/10 rounded-lg p-4 space-y-3">
                  <h4 className="text-xs font-extrabold text-pink-400 border-b border-gray-800 pb-1 uppercase tracking-wider font-sans">
                    👩 {t('porutham.girlDetails') || 'Girl Details'}
                  </h4>
                  <div className="space-y-2">
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Ascendant</span>
                      <span className="text-white font-medium">{result.girl_astro_details?.ascendant_sign || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Rasi (Moon)</span>
                      <span className="text-white font-medium">{result.girl_astro_details?.rasi || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Star</span>
                      <span className="text-amber-400 font-medium">{result.girl_astro_details?.nakshatra || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Birth Dasha</span>
                      <span className="text-violet-400 font-medium font-mono">{result.girl_astro_details?.birth_dasa || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Current Dasha</span>
                      <span className="text-amber-400 font-medium font-mono">{result.girl_astro_details?.current_dasa || '—'}</span>
                    </div>
                    {result.girl_astro_details?.lucky_gem?.length > 0 && (
                      <div className="flex justify-between py-1 border-b border-gray-900/40">
                        <span className="text-gray-500 font-bold font-sans">Lucky Gemstone</span>
                        <span className="text-emerald-400 font-medium">{result.girl_astro_details.lucky_gem.join(', ')}</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Boy Details */}
                <div className="bg-slate-950/45 border border-blue-500/10 rounded-lg p-4 space-y-3">
                  <h4 className="text-xs font-extrabold text-blue-400 border-b border-gray-800 pb-1 uppercase tracking-wider font-sans">
                    👦 {t('porutham.boyDetails') || 'Boy Details'}
                  </h4>
                  <div className="space-y-2">
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Ascendant</span>
                      <span className="text-white font-medium">{result.boy_astro_details?.ascendant_sign || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Rasi (Moon)</span>
                      <span className="text-white font-medium">{result.boy_astro_details?.rasi || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Star</span>
                      <span className="text-amber-400 font-medium">{result.boy_astro_details?.nakshatra || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Birth Dasha</span>
                      <span className="text-violet-400 font-medium font-mono">{result.boy_astro_details?.birth_dasa || '—'}</span>
                    </div>
                    <div className="flex justify-between py-1 border-b border-gray-900/40">
                      <span className="text-gray-500 font-bold font-sans">Current Dasha</span>
                      <span className="text-amber-400 font-medium font-mono">{result.boy_astro_details?.current_dasa || '—'}</span>
                    </div>
                    {result.boy_astro_details?.lucky_gem?.length > 0 && (
                      <div className="flex justify-between py-1 border-b border-gray-900/40">
                        <span className="text-gray-500 font-bold font-sans">Lucky Gemstone</span>
                        <span className="text-emerald-400 font-medium">{result.boy_astro_details.lucky_gem.join(', ')}</span>
                      </div>
                    )}
                  </div>
                </div>
